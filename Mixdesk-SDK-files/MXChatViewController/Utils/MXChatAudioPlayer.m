...
{
    static MXChatAudioPlayer *sharedInstance = nil;
...
-(void)playSongWithUrl:(NSString *)songUrl
{
    ...
    dispatch_async(dispatch_queue_create("playSoundFromUrl", NULL), ^{
        [self.delegate MXAudioPlayerBeiginLoadVoice];
...
-(void)playSoundWithData:(NSData *)soundData{
    ...
    [_player play];
    [self.delegate MXAudioPlayerBeiginPlay];
}
...
-(void)setupPlaySound{
    ...
    //接收关闭声音的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(audioPlayerDidInterrupt:) name:MXAudioPlayerDidInterruptNotification object:nil];
    //红外线感应监听
...
- (void)didFinishAudioPlay {
    ...
    [self.delegate MXAudioPlayerDidFinishPlay];
