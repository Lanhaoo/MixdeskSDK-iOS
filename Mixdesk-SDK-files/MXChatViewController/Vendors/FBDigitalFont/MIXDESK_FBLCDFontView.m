...
#import "MIXDESK_FBLCDFont.h"

#define UIColorFromRGB(rgbValue) [UIColor colorWithRed:((float)(((rgbValue) & 0xFF0000) >> 16))/255.0 green:((float)(((rgbValue) & 0xFF00) >> 8))/255.0 blue:((float)((rgbValue) & 0xFF))/255.0 alpha:1.0]

@interface MIXDESK_FBLCDFontView ()
@property (nonatomic, copy) NSArray *symbols;
@end

@implementation MIXDESK_FBLCDFontView
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.drawOffLine) {
        ...
        for (i = 0; i < [self.symbols count]; i++) {
            [MIXDESK_FBLCDFont drawSymbol:FBFontSymbol8
    ...
    for (i = 0; i < [self.symbols count]; i++) {
        [MIXDESK_FBLCDFont drawSymbol:[[self.symbols objectAtIndex:i] intValue]
                   edgeLength:self.edgeLength
                    lineWidth:self.lineWidth
                   startPoint:CGPointMake(x + i * l, y)
                    inContext:ctx];
