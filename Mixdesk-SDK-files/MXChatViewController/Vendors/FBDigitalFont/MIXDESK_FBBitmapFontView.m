#import "MIXDESK_FBBitmapFontView.h"
#import "MIXDESK_FBFontSymbol.h"
#import "MIXDESK_FBBitmapFont.h"

#define UIColorFromRGB(rgbValue) [UIColor colorWithRed:((float)(((rgbValue) & 0xFF0000) >> 16))/255.0 green:((float)(((rgbValue) & 0xFF00) >> 8))/255.0 blue:((float)((rgbValue) & 0xFF))/255.0 alpha:1.0]

@implementation MIXDESK_FBBitmapFontView
...
{
    _text = text;
    self.symbols = [MIXDESK_FBFontSymbol symbolsForString:text];
...
- (NSInteger)numberOfHorizontalDot
{
    ...
    for (NSNumber *number in self.symbols) {
        ...
        totalDotsFromSymbols += [MIXDESK_FBBitmapFont numberOfDotsWideForSymbol:symbol];
    }
...
//    CGFloat l = (self.edgeLength + self.margin) * (5 + self.numberOfPaddingDotsBetweenDigits);
    ...
    for (i = 0; i < [self.symbols count]; i++) {
        ...
        [MIXDESK_FBBitmapFont drawSymbol:[[self.symbols objectAtIndex:i] intValue]
        ...
        CGFloat numberWide = [MIXDESK_FBBitmapFont numberOfDotsWideForSymbol:[[self.symbols objectAtIndex:i] intValue]];
    ...
    [MIXDESK_FBBitmapFont drawBackgroundWithDotType:self.dotType 
                                      color:self.offColor 
                                 edgeLength:self.edgeLength 
                                     margin:self.margin 
