...
#import "MXAGEmojiKeyBoardView.h"
#import "MXAGEmojiPageView.h"
#import "UIImage+MXGenerate.h"
...
NSString *const MXRecentUsedEmojiCharactersKey = @"RecentUsedEmojiCharactersKey";


@interface MXAGEmojiKeyboardView () <UIScrollViewDelegate, MXAGEmojiPageViewDelegate>
...
@implementation MXAGEmojiKeyboardView
...
- (MXAGEmojiKeyboardViewCategoryImage)defaultSelectedCategory {
  ...
  return MXAGEmojiKeyboardViewCategoryImageFace;
}
...
- (NSArray *)imagesForSelectedSegments {
  ...
  dispatch_once(&onceToken, ^{
    ...
    for (MXAGEmojiKeyboardViewCategoryImage i = MXAGEmojiKeyboardViewCategoryImageFace;
         i <= MXAGEmojiKeyboardViewCategoryImageCharacters;
         ++i) {
...
- (NSMutableArray *)recentEmojis {
  NSArray *emojis = [[NSUserDefaults standardUserDefaults] arrayForKey:MXRecentUsedEmojiCharactersKey];
...
- (void)setRecentEmojis:(NSMutableArray *)recentEmojis {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:recentEmojis forKey:MXRecentUsedEmojiCharactersKey];
}

- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<MXAGEmojiKeyboardViewDataSource>)dataSource {
...
- (UIImage *)segImageWithCategory:(MXAGEmojiKeyboardViewCategoryImage)category {
    NSString *imageName = @"";
    switch (category) {
//        case MXAGEmojiKeyboardViewCategoryImageRecent:
//            imageName = @"AGEmojiFlags";
//            break;
        case MXAGEmojiKeyboardViewCategoryImageFace:
            imageName = @"AGEmojiPeople";
            break;
        case MXAGEmojiKeyboardViewCategoryImageBell:
            imageName = @"AGEmojiObjects";
            break;
        case MXAGEmojiKeyboardViewCategoryImageFlower:
            imageName = @"AGEmojiAnimals";
            break;
        case MXAGEmojiKeyboardViewCategoryImageCar:
            imageName = @"AGEmojiPlaces";
            break;
        case MXAGEmojiKeyboardViewCategoryImageCharacters:
            imageName = @"AGEmojiSymbols";
            break;
    }
    UIImage *img = [[MXAssetUtil imageFromBundleWithName:imageName] addContentInsect:UIEdgeInsetsMake(7, 7, 7, 7)];
    img = [img imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    return img;
}

- (UIImage *)segImageUnselectedWithCategory:(MXAGEmojiKeyboardViewCategoryImage)category {
    NSString *imageName = @"";
    switch (category) {
//        case MXAGEmojiKeyboardViewCategoryImageRecent:
//            imageName = @"AGEmojiFlags";
...
- (BOOL)requireToSetPageViewForIndex:(NSUInteger)index {
  ...
  for (MXAGEmojiPageView *page in self.pageViews) {
...
- (MXAGEmojiPageView *)synthesizeEmojiPageView {
  ...
  CGRect pageViewFrame = CGRectMake(0,
                                    ...
                                    CGRectGetHeight(self.emojiPagesScrollView.bounds));
  MXAGEmojiPageView *pageView = [[MXAGEmojiPageView alloc] initWithFrame: pageViewFrame
                                                backSpaceButtonImage:[MXAssetUtil imageFromBundleWithName:@"MXFileCancel"]
...
- (MXAGEmojiPageView *)usableEmojiPageView {
  MXAGEmojiPageView *pageView = nil;
  for (MXAGEmojiPageView *page in self.pageViews) {
...
- (void)setEmojiPageViewInScrollView:(UIScrollView *)scrollView atIndex:(NSUInteger)index {
  ...
  MXAGEmojiPageView *pageView = [self usableEmojiPageView];
...
- (void)emojiPageView:(MXAGEmojiPageView *)emojiPageView didUseEmoji:(NSString *)emoji {
  [self setInRecentsEmoji:emoji];
  [self.delegate emojiKeyBoardView:self didUseEmoji:emoji];
}

- (void)emojiPageViewDidPressBackSpace:(MXAGEmojiPageView *)emojiPageView {
