...
static CGFloat const kMXPageScrollBottomLineHeight = 1.0;
static CGFloat const kMXPageScrollItemLeftAndRightMargin = 0.0;
static CGFloat const kMXPageScrollItemMargin = 15.0;

@interface MXPageScrollMenuView()
...
@implementation MXPageScrollMenuView
...
- (void)updateScrollOffsetWithIndex:(NSInteger)index willIndex:(NSInteger)willIndex indexPercent:(float)percent {
    ...
    if (willIndex > index) {
        self.bottomLine.frame = CGRectMake(CGRectGetMinX(currentButton.frame) + CGRectGetWidth(currentButton.frame) * percent, CGRectGetMaxY(currentButton.frame), CGRectGetWidth(currentButton.frame) * (1 - percent) + CGRectGetWidth(willButton.frame) * percent,kMXPageScrollBottomLineHeight);
        [self adjustItemPositionWithCurrentIndex:willIndex];
    } else {
        self.bottomLine.frame = CGRectMake(CGRectGetMaxX(willButton.frame) - CGRectGetWidth(willButton.frame) * percent, CGRectGetMaxY(currentButton.frame), CGRectGetWidth(currentButton.frame) * (1 - percent) + CGRectGetWidth(willButton.frame) * percent,kMXPageScrollBottomLineHeight);
        [self adjustItemPositionWithCurrentIndex:willIndex];
    }
...
- (void)setupOtherViews {
    ...
    __block CGFloat itemH = self.bounds.size.height - kMXPageScrollBottomLineHeight;
    
    [self.itemsArrayM enumerateObjectsUsingBlock:^(UIButton * _Nonnull button, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx == 0) {
            itemX += kMXPageScrollItemLeftAndRightMargin;
        }else{
            itemX += kMXPageScrollItemMargin + [self.itemsWidthArraM[idx - 1] floatValue];
        }
        button.frame = CGRectMake(itemX, itemY, [self.itemsWidthArraM[idx] floatValue], itemH);
    }];
    
    CGFloat scrollSizeWidth = kMXPageScrollItemLeftAndRightMargin + CGRectGetMaxX([[self.itemsArrayM lastObject] frame]);
    self.scrollView.contentSize = CGSizeMake(scrollSizeWidth, self.scrollView.bounds.size.height);
    
    self.bottomLine.frame = CGRectMake(kMXPageScrollItemLeftAndRightMargin, CGRectGetMaxY(self.scrollView.frame) - kMXPageScrollBottomLineHeight, [self.itemsWidthArraM.firstObject floatValue], kMXPageScrollBottomLineHeight);
...
- (void)adjustItemAnimate:(BOOL)animated {
    ...
    [UIView animateWithDuration:animated ? 0.3 : 0 animations:^{
        ...
        currentButton.selected  = YES;
        self.bottomLine.frame = CGRectMake(CGRectGetMinX(currentButton.frame), CGRectGetMaxY(currentButton.frame), CGRectGetWidth(currentButton.frame), kMXPageScrollBottomLineHeight);
        self.lastIndex = self.currentIndex;
