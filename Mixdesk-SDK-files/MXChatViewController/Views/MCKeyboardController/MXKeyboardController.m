...
static void * kMXKeyboardControllerKeyValueObservingContext = &kMXKeyboardControllerKeyValueObservingContext;

@interface MXKeyboardController () <UIGestureRecognizerDelegate>
...
@implementation MXKeyboardController
                          ...
                          delegate:(id<MXKeyboardControllerDelegate>)delegate
{
    ...
    if (self = [self init]) {
        ...
        _delegate = delegate;
        _keyboardStatus = MXKeyboardStatusHide;
    }
    return self;
}
    ...
    if (keyboardView && !_isObserving) {
        [_keyboardView addObserver:self
                        forKeyPath:NSStringFromSelector(@selector(frame))
                           ...
                           context:kMXKeyboardControllerKeyValueObservingContext];
...
//                                self.keyboardView = subsubView;
    ...
    self.keyboardStatus = MXKeyboardStatusWillShow;
    [self setKeyboardViewHidden:NO];
...
{
    self.keyboardStatus = MXKeyboardStatusShowing;
...
{
    if (self.keyboardStatus == MXKeyboardStatusHide) {
        self.keyboardStatus = MXKeyboardStatusWillShow;
    }
    [self handleKeyboardNotification:notification completion:nil];
}
...
{
    self.keyboardStatus = MXKeyboardStatusWillHide;
}

- (void)didReceiveKeyboardDidHideNotification:(NSNotification *)notification
{
    self.keyboardStatus = MXKeyboardStatusHide;
    self.keyboardView = nil;
...
{
    if (context == kMXKeyboardControllerKeyValueObservingContext) {
