...
@interface MXEmbededWebView()<WKNavigationDelegate>
...
@implementation MXEmbededWebView
...
//    }
    ...
    static  NSString * const jsGetImages =
        ...
        imgScr = imgScr + objs[i].src +'MXindex'+ i +'M+Q';\
        (function(arg){\
        objs[arg].onclick=function(){\
        document.location=\"myweb:imageClick:\"+this.src + 'MXindex' + arg;\
        };\
        })(i); \
        };\
        return imgScr;\
...
- (void)getFinalImagesUrls {
    ...
    for (int i = 0; i < self.allUrlArray.count; i++) {
        NSArray *imageIndex = [NSMutableArray arrayWithArray:[self.allUrlArray[i] componentsSeparatedByString:@"MXindex"]];
...
- (void)showBigImage:(NSString *)imageUrl {
    ...
    NSArray *imageIndex = [NSMutableArray arrayWithArray:[imageUrl componentsSeparatedByString:@"MXindex"]];
    ...
    [viewerVC showOn:[UIViewController mx_topMostViewController] fromRectArray:[NSArray arrayWithObject:[NSValue valueWithCGRect:[self convertRect:self.frame toView:[UIApplication sharedApplication].keyWindow]]]];
}
@end
