...
@interface MXImageViewerViewController ()<UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
@implementation MXImageViewerViewController
...
//        [tempImageArray addObject:[MXImageUtil resizeImageToMaxScreenSize:image]];
//    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    MXImageCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:KCellReuseId forIndexPath:indexPath];
...
- (void)loadImageForIndexPath:(NSIndexPath *)indexPath forCell:(MXImageCollectionCell *)cell {
    NSString *imagePath = self.imagePaths[indexPath.row];
    if (self.images.count == 0) {
        [MXServiceToViewInterface downloadMediaWithUrlString:imagePath progress:nil completion:^(NSData *mediaData, NSError *error) {
            cell.imageView.image = [MXImageUtil resizeImageToMaxScreenSize:[UIImage imageWithData:mediaData]];
        }];
    } else {
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[MXImageCollectionCell class] forCellWithReuseIdentifier:KCellReuseId];
    }
...
- (UIButton *)saveButton {
    if (!_saveButton) {
        ...
        [_saveButton setTitle:[MXBundleUtil localizedStringForKey:@"save_photo"] forState:(UIControlStateNormal)];
...
- (void)saveImage {
    ...
    UIImage *image = [(MXImageCollectionCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]] imageView].image;
    if (image) {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *) error contextInfo:(void *)contextInfo {
    if (error) {
        [MXToast showToast:[MXBundleUtil localizedStringForKey:@"save_photo_error"] duration:0.25 window:self.view.window];
    } else {
        [MXToast showToast:[MXBundleUtil localizedStringForKey:@"save_photo_success"] duration:0.25 window:self.view.window];
    }
}
...
#pragma mark - ***
#pragma mark - MXImageCollectionCell

@interface MXImageCollectionCell() <UIScrollViewDelegate>
...
@implementation MXImageCollectionCell
