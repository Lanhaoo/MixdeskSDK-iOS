...
@interface MXRecorderView() //<MXChatAudioRecorderDelegate>



@end

@implementation MXRecorderView
{
    UIView *recordButton;
    UIImageView *micImageView;
    CALayer *volumeLayer;
    
    MXChatAudioRecorder *recorder;
...
- (void)setupUI {
    ...
    _tipLabel.text = [MXBundleUtil localizedStringForKey:@"mx_audio_status_normal"];
    ...
    micImageView = [[UIImageView alloc] initWithImage:[MXAssetUtil imageFromBundleWithName:@"rectangle9Copy5"]];
...
- (void)changeVolumeLayerDiameter:(CGFloat)dia_ {
    ...
    if (time >= 50) {
        if (point.y < 50) {
            self.tipLabel.text = [NSString stringWithFormat:[MXBundleUtil localizedStringForKey:@"mx_record_count_down"], (long)(60 - time)];
        }else{
            self.tipLabel.text = [NSString stringWithFormat:[MXBundleUtil localizedStringForKey:@"mx_record_will_send"], (long)(60 - time)];
        }
    }
}
...
- (void)panGestureRecognizerAction:(UIGestureRecognizer*)sender {
    ...
    if(sender.state == UIGestureRecognizerStateBegan) {
        ...
        self.tipLabel.text = [MXBundleUtil localizedStringForKey:@"record_cancel_swipe"];
        ...
        if (point.y < 50) {
            ...
            if ([self getRecordTime] < 1) {
                [MXToast showToast:[MXBundleUtil localizedStringForKey:@"recode_time_too_short"] duration:0.2 window:self.window];
                if ([self.delegate respondsToSelector:@selector(recordCanceld)]) {
                    [self.delegate recordCanceld];
                }
            }else{
        ...
        [self reUI];
            ...
            if (point.y < 50) {
                if ([self getRecordTime] < 50) {
                    self.tipLabel.text = [MXBundleUtil localizedStringForKey:@"record_cancel_realse"];
                }
                micImageView.image = [MXAssetUtil imageFromBundleWithName:@"exit_recording"];
                recordButton.backgroundColor = [UIColor colorWithRed: 150/255.0 green: 159/255.0 blue: 170/255.0 alpha: 1];
            }else{
                if ([self getRecordTime] < 50) {
                    self.tipLabel.text = [MXBundleUtil localizedStringForKey:@"record_cancel_swipe"];
                }
                micImageView.image = [MXAssetUtil imageFromBundleWithName:@"rectangle9Copy5"];
                recordButton.backgroundColor = [UIColor colorWithRed: 23/255.0 green: 199/255.0 blue: 209/255.0 alpha: 1];
            }
    }
}
...
- (void)reUI {
    ...
    self.tipLabel.text = [MXBundleUtil localizedStringForKey:@"mx_audio_status_normal"];
    micImageView.image = [MXAssetUtil imageFromBundleWithName:@"rectangle9Copy5"];
    recordButton.backgroundColor = [UIColor colorWithRed: 23/255.0 green: 199/255.0 blue: 209/255.0 alpha: 1];
}
