...
@class MXButtonGroupBar;

/**
 *  「MCButtonGroupBarDelegate」协议定义的方法是与「MCButtonGroupBar」对象进行交互的
 */
@protocol MXButtonGroupBarDelegate <NSObject>
 ...
 */
- (void)buttonGroup:(nonnull MXButtonGroupBar *)buttonGroupBar
     didClickButton:(nonnull UIButton *)sender
     formGroupIndex:(NSUInteger)index;

@end

@interface MXButtonGroupBar : UIView
...
/** 这个对象充当按钮组栏的委托 */
@property (weak, nonatomic, nullable  ) id<MXButtonGroupBarDelegate> delegate;
 ...
 *  @return 如果初始化成功，将是一个「MCButtonGroupBar」对象，否则为 nil
 */
- (_Nonnull instancetype)initWithFrame:(CGRect)frame buttons:(NSArray * _Nonnull)buttons delegate:(id<MXButtonGroupBarDelegate> _Nullable)delegate;
